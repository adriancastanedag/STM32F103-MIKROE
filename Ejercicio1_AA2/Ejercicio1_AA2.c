...
2) Conmute el LED cuando el BOTON DE USUARIO no se encuentre a presionado
  Se tiene que definir que significa que el boton no este presionado y se declara en el segmento
  2 de nuestro código
  #define NOPRESIONADO     1
  Se modifica la función proceso en su condicional if(BOTON==NOPRESIONADO)
*/
...
/*
   ...
   Descripción
   La función proceso verifica si el boton esta presionado  y si lo esta conmuta
   el valor del led y llama a una función de delay con retardo prefedinido
   Parametros
